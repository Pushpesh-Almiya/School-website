...
function Navbar() {
  ...
  <nav className="container mx-auto flex justify-between lg:justify-between items-center text-gray-100">
    ...
    <ul
      className={`${
        isOpen ? "block" : "hidden"
      } lg:flex flex-col my-4 lg:my-0 pb-2 lg:flex-row ml-auto space-y-4 lg:space-y-0 lg:space-x-4 absolute lg:static bg-[#BF2EF0] lg:bg-transparent left-0 top-16 lg:top-auto w-full lg:w-auto z-10 lg:z-auto transition-transform duration-300`}
    >
      {navItems.map((item) => (
        <ScrollLink
          key={item.slug}
          ...
          className="font-semibold text-lg px-4 lg:py-2 md:px-0 text-[#FFF6EA] transition-colors duration-200 relative flex items-center justify-center"
        >
          <span
          ...
          >
            <span className="m-2">{item.icon}</span> {/* Added margin here */}
            <span>{item.name}</span>
          </span>
        </ScrollLink>
      ))}
